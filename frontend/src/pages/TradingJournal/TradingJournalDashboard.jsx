...
const TradingJournalDashboard = () => {
  ...
  // ============================================================
  // SET LATEST TRADING PERIOD
  // ONLY USED WHEN THERE IS NO SELECTED PERIOD
  // OR THE OLD "ALL" PERIOD IS STILL SAVED.
  // ============================================================
  ...
  useEffect(() => {
    ...
    const date = new Date(latestTrade.openDate);

    const latestPeriod = `${date.getFullYear()}-${String(
      date.getMonth() + 1,
    ).padStart(2, "0")}`;

    setSelectedPeriod(latestPeriod);
  }, [trades, tradesLoading, selectedPeriod, setSelectedPeriod]);
  ...
  const periodTrades = trades.filter((trade) => {
    if (!trade.openDate || !selectedPeriod || selectedPeriod === "ALL") {
      return false;
    }
